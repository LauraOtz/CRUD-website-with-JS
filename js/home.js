...
if (usuario) {
  if (usuario.rol === "admin") {
    ...
    let opcion = ` <a class="nav-link text-dark" aria-current="page" href="./admin.html"
     >Administración</a>`;
...
if (usuario) {
  if (usuario.rol === "usuario") {
    let item = document.createElement("li");
    item.classList = "nav-item";
    let mayus= usuario.nombre
    let opcion = ` <a class="nav-link text-dark"  aria-current="page"
        >Hola ${mayus}</a>`;
    item.innerHTML = opcion;
  ...
  if (cursos.length > 0) {
    ...
    cursos.map(function (curso) {
      ...
      <div class="card-body">
        ...
        <a href="#" class="btn btn-color float-end">Comprar</a>
        </div>
      </div>`;
